...
if defined?(Rails::Console) && Rails.env.development?
  ...
  def model_stats
    Rails.logger.debug "\e[1;34m=== Model Statistics ===\e[0m"
    models = [User, Stream, Streamer, StreamerAccount, Timestamp, Location]
    models.each do |model|
      count = model.count
      color = count.positive? ? "\e[1;32m" : "\e[1;33m"
      Rails.logger.debug { "#{color}#{model.name.ljust(20)}\e[0m: #{count}" }
    end
    nil
  end

  # Welcome message
  Rails.logger.debug { "\e[1;35m#{'=' * 50}\e[0m" }
  Rails.logger.debug "\e[1;35mWelcome to StreamSource Rails Console!\e[0m"
  Rails.logger.debug { "\e[1;35m#{'=' * 50}\e[0m" }
  Rails.logger.debug { "\e[1;33mEnvironment:\e[0m #{Rails.env}" }
  Rails.logger.debug { "\e[1;33mDatabase:\e[0m #{ActiveRecord::Base.connection.current_database}" }
  Rails.logger.debug ""
  Rails.logger.debug "\e[1;36mUseful commands:\e[0m"
  Rails.logger.debug "  \e[0;32mmodel_stats\e[0m - Show record counts for all models"
  Rails.logger.debug "  \e[0;32mreload!\e[0m     - Reload the console"
  Rails.logger.debug ""
end
