  ...
  describe 'Stream Analytics' do
    context 'when feature is disabled' do
      ...
      it 'returns forbidden for analytics endpoint' do
        ...
        expect(JSON.parse(response.body)['error']).to eq('This feature is not currently available')
      end
  ...
  describe 'Percentage-based rollout' do
    ...
    it 'is enabled for approximately half of users' do
      ...
      100.times do |i|
        ...
        if Flipper.enabled?(ApplicationConstants::Features::AI_STREAM_RECOMMENDATIONS, test_user)
          enabled_count += 1
        end
      end
