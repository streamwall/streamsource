...
RSpec.describe Streamer, type: :model do
  ...
  describe "associations" do
    ...
    it { is_expected.to have_many(:streamer_accounts).dependent(:destroy) }
    it { is_expected.to have_many(:streams).dependent(:destroy) }
    it { is_expected.to have_many(:timestamp_streams).through(:streams) }
  ...
  describe "scopes" do
    ...
    describe ".with_active_accounts" do
      it "returns streamers with active accounts" do
        ...
        expect(described_class.with_active_accounts).to include(streamer)
        expect(described_class.with_active_accounts).not_to include(other_streamer)
      end
    end
    ...
    describe ".by_platform" do
      it "returns streamers by platform" do
        ...
        expect(described_class.by_platform("twitch")).to include(streamer)
        expect(described_class.by_platform("twitch")).not_to include(other_streamer)
      end
    end
    ...
    describe ".with_live_streams" do
      it "returns streamers with live streams" do
        ...
        expect(described_class.with_live_streams).to include(streamer)
        expect(described_class.with_live_streams).not_to include(other_streamer)
      end
    end
    ...
    describe ".recently_active" do
      it "returns streamers with recent streams" do
        ...
        expect(described_class.recently_active).to include(streamer)
        expect(described_class.recently_active).not_to include(other_streamer)
      end
    end
    ...
    describe ".search" do
      it "searches by name" do
        ...
        expect(described_class.search("Game")).to include(streamer1)
        expect(described_class.search("Game")).not_to include(streamer2)
      end

      it "is case insensitive" do
        streamer = create(:streamer, name: "GamePlayer")
        expect(described_class.search("game")).to include(streamer)
      end
    end
  end
  ...
  describe "instance methods" do
    ...
    describe "#last_stream" do
      it "returns the most recent stream by started_at" do
        create(:stream, streamer: streamer, started_at: 2.days.ago)
    ...
    describe "#account_for_platform" do
      it "returns active account for platform" do
        ...
        create(:streamer_account, streamer: streamer, platform: "youtube", is_active: false)
