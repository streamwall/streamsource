...
RSpec.describe Location, type: :model do
  ...
  describe 'validations' do
    ...
    it { should validate_presence_of(:normalized_name) }
    
    describe 'uniqueness of normalized_name' do
      subject { create(:location) }
      it { should validate_uniqueness_of(:normalized_name) }
    end
  end
  ...
  describe 'scopes' do
    ...
    describe '.ordered' do
      it 'orders by city and state_province' do
        expect(Location.ordered).to eq([uk_location, usa_location, canada_location])
      end
    end
    
    describe '.by_country' do
      it 'filters by country' do
        expect(Location.by_country('USA')).to include(usa_location)
        expect(Location.by_country('USA')).not_to include(uk_location, canada_location)
      end
      
      it 'returns all when country is blank' do
        expect(Location.by_country('')).to match_array([usa_location, uk_location, canada_location])
      end
    end
    
    describe '.by_state' do
      it 'filters by state_province' do
        expect(Location.by_state('NY')).to include(usa_location)
        expect(Location.by_state('NY')).not_to include(uk_location, canada_location)
      end
    end
    
    describe '.search' do
      it 'searches in city' do
        expect(Location.search('york')).to include(usa_location)
        expect(Location.search('london')).to include(uk_location)
      end
      
      it 'searches in state_province' do
        expect(Location.search('NY')).to include(usa_location)
        expect(Location.search('ON')).to include(canada_location)
      end
      
      it 'searches in country' do
        expect(Location.search('USA')).to include(usa_location)
        expect(Location.search('canada')).to include(canada_location)
      end
      
      it 'is case insensitive' do
        expect(Location.search('NEW YORK')).to include(usa_location)
        expect(Location.search('usa')).to include(usa_location)
      end
      
      it 'returns all when query is blank' do
        expect(Location.search('')).to match_array([usa_location, uk_location, canada_location])
      end
    end
  end
  ...
  describe '.find_or_create_from_params' do
    context 'when location exists' do
      ...
      it 'finds existing location by normalized name' do
        ...
        location = Location.find_or_create_from_params(params)
    ...
    context 'when location does not exist' do
      it 'creates new location' do
        ...
        expect {
          Location.find_or_create_from_params(params)
    ...
    context 'when city is blank' do
      it 'returns nil' do
        ...
        expect(Location.find_or_create_from_params(params)).to be_nil
      end
    end
  end
  ...
  describe '.normalize_location_name' do
    it 'normalizes location parts' do
      expect(Location.normalize_location_name('New York', 'NY', 'USA')).to eq('new york, ny, usa')
    end
    
    it 'handles nil values' do
      expect(Location.normalize_location_name('Paris', nil, 'France')).to eq('paris, france')
      expect(Location.normalize_location_name('Tokyo', nil, nil)).to eq('tokyo')
    end
    
    it 'strips whitespace' do
      expect(Location.normalize_location_name('  London  ', '  ', 'UK  ')).to eq('london, uk')
    end
  end
