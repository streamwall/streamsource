...
RSpec.describe TimestampStream, type: :model do
  ...
  describe 'scopes' do
    ...
    let!(:with_timestamp) { create(:timestamp_stream, timestamp: timestamp, stream: create(:stream), stream_timestamp_seconds: 120) }
    let!(:without_timestamp) { create(:timestamp_stream, timestamp: timestamp, stream: create(:stream), stream_timestamp_seconds: nil) }
    
    describe '.with_timestamp' do
      it 'returns only timestamps with timestamps' do
        expect(TimestampStream.with_timestamp).to include(with_timestamp)
        expect(TimestampStream.with_timestamp).not_to include(without_timestamp)
      end
    end
  end
  ...
  describe 'database indexes' do
    it 'has required indexes' do
      expect(ActiveRecord::Base.connection.index_exists?(:timestamp_streams, [:timestamp_id, :stream_id])).to be true
