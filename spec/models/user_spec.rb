...
RSpec.describe User, type: :model do
  ...
  describe "associations" do
    ...
    it { is_expected.to have_many(:streamers).dependent(:destroy) }
    it { is_expected.to have_many(:timestamps).dependent(:destroy) }
    it { is_expected.to have_many(:timestamp_streams).with_foreign_key("added_by_user_id").dependent(:destroy) }
  end
  ...
  describe "enums" do
    it { is_expected.to define_enum_for(:role).backed_by_column_of_type(:string).with_values(default: "default", editor: "editor", admin: "admin") }
  ...
  describe "scopes" do
    ...
    describe ".editors" do
      it "returns only editors" do
        expect(described_class.editors).to contain_exactly(editor)
      end
    end
  ...
  describe "edge cases" do
    ...
    it "prevents SQL injection in email" do
      ...
      user.valid?
      expect(described_class.count).to be >= 0 # Table should still exist
    end
    ...
    it "handles role changes during concurrent access" do
      ...
      user1 = described_class.find(user.id)
