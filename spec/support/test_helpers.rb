module TestHelpers
  ...
  def create_resources_with_attributes(factory, count, attributes_list)
    attributes_list.take(count).map.with_index do |attrs, i|
      create(factory, attrs)
    end
  end
  
  # Helper to test filtering
  def test_filter(path, filter_param, filter_value, expected_count, &block)
  ...
  def test_sorting(path, sort_param, resources, &comparison_block)
    get path, params: { sort: sort_param }
    expect_json_success
    
    results = json_response['data']
    sorted_ids = results.map { |r| r['id'] }
    expected_ids = resources.sort_by(&comparison_block).map(&:id)
  ...
  def expect_to_change_value(object, attribute, from:, to:)
    expect {
      yield
      object.reload
    }.to change { object.send(attribute) }.from(from).to(to)
  end
  ...
  def expect_not_to_change_value(object, attribute)
    expect {
      yield
      object.reload
    }.not_to change { object.send(attribute) }
  end
  ...
  def test_async_operation(path, method, params, &verification_block)
    ...
    verification_block.call if block_given?
  end
end
